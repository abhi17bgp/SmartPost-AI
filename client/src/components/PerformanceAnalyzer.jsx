    ...
    if (testType === 'sequential') {
      for (let i = 1; i <= runs; i++) {
        try {
          ...
          testResults.push({ run: i, time: timeTaken, error: false });
        } catch (err) {
          testResults.push({ run: i, time: null, error: true });
        }
      ...
      // Parallel execution
      const parallelResults = Array.from({ length: runs }).map((_, i) => ({ run: i + 1, pending: true }));
